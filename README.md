...
| **v4** | `specification/Efficiency_Analysis.MD` — v3 не выражал сменные графики: неделя из 7 строк не описывает цикл 2/2 | Цикл произвольной длины 2–31 сутки, бригады со сдвигом, единый резолвер, календарь результата |
| **v5** | `specification/Comparision_with_other_SW.MD` — v4 сложнее шести продуктов рынка в массовом сценарии и беднее в особых днях | Тип расписания решает сложность редактора; праздники и исключения слиты в «Особые дни»; объяснение результата словами; проверка лимитов контроллеров |
| **v6** | Панели самопроизвольно прокручивались в начало; плюс переход на визуальное семейство ONE PSIM | Прокрутка переживает перерисовку, периодическое обновление стало точечным; стили приведены к ONE PSIM файлом `bridge.css` |
...
prototypes/          — сами прототипы и всё, что им нужно для работы
    schedule_v6.html     актуальная версия
    schedule_v5.html     schedule_v4.html     schedule_v3.html
    bridge.css           приведение v6 к визуальному семейству ONE PSIM
    theme/               токены и нативные контролы, копии из ONE.PSIM.PrototypeKit
specification/       — все документы: анализы, описания версий, сравнение с рынком
screenshots/         — снимки состояний всех версий
sources/             — исходные материалы, с которых всё начиналось (не наши прототипы)
tests/               — проверочные скрипты
```

`bridge.css` и `theme/` лежат **рядом с прототипом намеренно**: `schedule_v6.html` подключает
`bridge.css` относительной ссылкой, а тот импортирует `theme/*.css` — папку `prototypes/` можно
скопировать или отправить целиком, и версия v6 будет работать.

---

## Быстрый старт

Откройте **`prototypes/schedule_v6.html`** двойным кликом — рядом лежащие `bridge.css` и `theme/`
подтянутся сами. Устанавливать и собирать ничего не нужно.

Предыдущие версии самодостаточны: `schedule_v5.html`, `schedule_v4.html`, `schedule_v3.html` — один
файл без внешних зависимостей, их можно открывать откуда угодно.
...
Снимки ниже сняты с v5, то есть **до** перехода на ONE PSIM: компоновка у v6 та же, отличаются
цвета, шрифт, радиусы и состояния контролов. Снимки v6 — отдельный шаг.
...
![Недельное расписание, английская локаль](screenshots/v5-01-weekly-en.png)
...
![Сменный цикл, русская локаль](screenshots/v5-02-cycle-ru.png)
...
![Астрономическое расписание](screenshots/v5-03-astro-en.png)
...
![Круглосуточное расписание, тёмная тема](screenshots/v5-07-always-dark.png)

Остальные снимки — в папке `screenshots/`: отчёт о совместимости, тип «по датам» с предпросмотром
...
| Файл | Что это |
|---|---|
| **`schedule_v6.html`** | **Актуальный прототип.** Требует рядом `bridge.css` и `theme/` |
| `bridge.css` | Приведение к визуальному семейству ONE PSIM; первыми строками импортирует обе таблицы из `theme/` |
| `theme/` | Словарь токенов и нативные контролы, копии из `ONE.PSIM.PrototypeKit` |
| `schedule_v5.html` | Предыдущая версия: типы расписаний, особые дни, совместимость |
| `schedule_v4.html` | Цикл произвольной длины, бригады, календарь |
...
| `BRIDGE-README.md` | Переход на ONE PSIM: диагностика, карта токенов, исправленные лестницы, свои решения, вопросы владельцу, пробелы правил. Там же разбор дефекта с прокруткой |

### `sources/` — исходные материалы

Это то, с чего начинали, а не наши прототипы; лежат отдельно, чтобы их не путали с версиями.
...
| Файл | Что это |
|---|---|
| `v5-*.png` | Снимки состояний v5 в двух локалях и обеих темах |
| `v6-menu-*.png`, `v6-select-*.png`, `v6-state-*.png` | Снимки v6 после перехода на ONE PSIM: открытые меню, попап `<select>`, состояния кнопок с реальным наведением |
| `check-*.png` | Снимки, которые делает `npm run check` кита: light/dark 1440, dark 420 |
| `v4_en.png`, `v4_ru.png`, `v3_en.png`, `v3_ru.png`, `schedule_v3_preview.png` | Превью предыдущих версий |

Кит пишет свои снимки в `<папка прототипа>/shots/`, то есть в `prototypes/shots/` — оттуда их
нужно переносить сюда, чтобы все снимки лежали в одном месте.

### `tests/`

Проверочные скрипты: функциональные тесты прототипа, проверка `bridge.css`, контраст, сверка
расчёта солнца с NOAA, метрики версий. Запуск — из корня проекта, см. раздел «Проверка».
...
```
node tests/test_v5.js schedule_v6.html   # 111 функциональных проверок (файл — аргумент)
node tests/test_v6_scroll.js             # 40 проверок: прокрутка переживает перерисовку
node tests/check_bridge.js               # 29 проверок bridge.css: токены, селекторы, §2.9a′
node tests/check_contrast.js             # контраст всех пар «текст на заливке»
node tests/check_sun_vs_noaa.js          # сверка расчёта солнца с формулами NOAA
node tests/check_links.js                # все пути в документах и прототипах существуют
node tests/metrics.js                    # метрики версий
```

Скрипты сами находят прототипы в `prototypes/`, запускать их нужно из корня проекта.

Функциональные тесты подставляют заглушки DOM и проверяют резолвер для всех типов расписаний,
приоритеты правил, порядковые правила, арифметику интервалов, лимиты оборудования, объяснения на
двух языках, форму запроса к API и полноту словаря.
