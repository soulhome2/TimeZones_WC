...
Откройте **`schedule_v6.html`** двойным кликом. Рядом должны лежать `bridge.css` и папка `theme/` —
это стили ONE PSIM; всё остальное прототип несёт в себе, устанавливать и собирать ничего не нужно.
...
| `bridge.css` | Приведение к визуальному семейству ONE PSIM; импортирует `theme/one-psim.css` и `theme/native.css` |
...
```
node _dev/test_v5.js schedule_v6.html   # 111 функциональных проверок (файл — аргумент)
node _dev/test_v6_scroll.js             # 40 проверок: прокрутка переживает перерисовку
node _dev/check_bridge.js               # 13 проверок bridge.css: токены и селекторы
node _dev/check_contrast.js             # контраст всех пар «текст на заливке»
